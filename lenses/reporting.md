...
A **theme** is the report's visual skin — palette, light/dark, fonts, an optional logo — and it is the **one cosmetic choice that legitimately belongs to the user or the organisation**, not to the data. It is *orthogonal* to everything else this lens decides: the **shape** and the **chart types** are fixed by the analytical type and the audience (above, and in [`../lenses/data-analysis.md`](../lenses/data-analysis.md)), and a re-skin must never change which structure or which chart carries the finding. The theme paints the report; it does not reshape it.

This reconciles the lens's stance against gratuitous chrome (see "When to Use" — *do not add chrome, logos, cover pages that serve no reader*) with the reality that an official report often *should* carry an organisation's identity. The distinction is **subordination**: a brand skin is fine when the deliverable's context expects it and it stays below the data — the conclusion still leads, the provenance still shows, and the chart's colours still obey honest encoding (a brand palette does not earn a truncated axis or a pie for a trend). Chrome that pushes the conclusion below the fold, a cover page that delays the answer, or a logo larger than the finding is the failure the anti-chrome rule names; a small mark and a brand palette that stay out of the data's way are not.

The theme is a **preset chosen at the render step**, and several can coexist (a dense dark dashboard, a clean light report, a minimal executive skin). A consuming skill **suggests** a theme from the AnalysisSpec's audience and purpose — a steering-group exec-summary leans to a clean minimal skin; an ops dashboard to a dense dark one — and the **user confirms or changes it**: the theme is a shared decision, the one place a menu is appropriate. **The chart type and the report shape are never a menu** — offering "which chart would you like?" for the same data is exactly the distortion the discipline forbids; the agent picks the chart the evidence demands and lets the user pick only the skin. Mechanically, themes are CSS-variable presets and the chart colours read the same variables so the data re-skins honestly — the contract and neutral examples live in [`../templates/THEMING.md`](../templates/THEMING.md).
...
- **Theme overriding substance** — *Diff:* a brand skin that buries the conclusion under a cover page or an oversized logo, recolours a chart so its encoding misleads, or a render that offers the *chart type* or the *report shape* as a cosmetic menu. *Harm:* the skin competes with — or distorts — the finding; a reader picks a pretty chart that does not match the data, and the report persuades of something the analysis does not support. *Fix:* keep the theme subordinate — the conclusion still leads, the provenance still shows, the charts still obey honest encoding — and offer only the *theme* as a choice, never the chart type or the shape, which follow the data. See "Theming & brand identity" above and [`../templates/THEMING.md`](../templates/THEMING.md).
