...
- **`sources`** (each with **owner** and a **freshness** attribute) — from Sources & Ownership and Freshness: every system/object/environment named, its owner, the freshness attribute of each `sources` entry (its last-load time and lag), and *how* it was obtained (method, never the secret).
