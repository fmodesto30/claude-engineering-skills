...
This lens has two consumers, used with opposite intents. **`architecture-review`** reads it **evaluatively**, at **system/design altitude** — boundaries, model integrity, consistency rules, data ownership, and how the design will evolve — not line-by-line PR nitpicking. The review-flavored sections below (severity calibration, comment style, integration contract) are written for that design-review intent: *do the boundaries and the model hold together as the system changes, or is the design either leaking across contexts or paying for DDD machinery it does not need?*

That shared use — evaluative for `architecture-review`, generative for `spec-author` (which reads this lens to *decide* boundaries, aggregates, and the consistency boundary when writing a spec, rather than to judge them after the fact) — is exactly why this knowledge lives in `lenses/` rather than inside one skill: each consumer supplies its own intent, the lens supplies the knowledge. It stays focused on the concrete decisions both intents can act on, not over-generalized. A consuming skill loads it only when the design or diff actually touches the structural areas in "When to Use" below — never just because the words "aggregate" or "bounded context" appear in a name, a package, or a design doc.
